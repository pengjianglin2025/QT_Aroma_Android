<template>
	<view class="mine-container" :style="{height: `${windowHeight}px`}">
		...
		<view class="action-btn">
			<button @click="handleLogout" class="login-btn block lg" style="background-color: #01CBA5;">{{$t('logout')}}</button>
		</view>
		<view class="action-btn">
			<button @click="deleteAccount" class="login-btn block lg">{{$t('login.accountDeletion')}}</button>
		</view>
...
<script>
	...
	export default {
		...
		computed: {
			...
			// 	return uni.getStorageSync('avatarUrl') || '../../static/circle-style.png'
			// },
			windowHeight() {
		...
		onShow() {
			...
			let that = this
			that.avatar = uni.getStorageSync('avatarUrl')?  '../..'+ uni.getStorageSync('avatarUrl'): '../../static/circle-style.png'
			// console.log('that.avatar===='+that.avatar);
			that.getUserInfo()
			uni.getSystemInfo({
				success: function(res) {
					console.log(res)
					that.version = res.appWgtVersion
		...
		methods: {
			...
			deleteAccount(){
			  let _this = this
			  uni.showModal({
				// title: _this.$t('com.tip'),
				// content: _this.$t('login.deletionTips'),
				title: _this.$t('login.deletionTips'), 
				content: _this.$t('login.deletionTipsDetails'),
				cancelText: _this.$t('com.cancel'),
				confirmText: _this.$t('com.confirm'),
				success: function(res) {
				  if (res.confirm) {
					deleteUser({type:2}).then(res=>{
