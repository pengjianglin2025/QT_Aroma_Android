<template>
	<view>
			...
			<view style="position: relative;">
				...
				<view v-if="parameterMode==1">
                    ...
                    <view v-else class="tcon">
                        <div>{{$t('remain-time') === 'remain-time' ? '剩余时间' : $t('remain-time')}}</div>
		...
		<view class="flex">
			<button size="mini" @click="clearLogger">娓呴櫎鏃ュ織</button>
	...
	export default {
		...
		computed:{
			...
			currentModeDisplay(){
				...
				if(this.curModeMsg && this.curModeMsg.modeNum){
					return '\u4e13\u4e1a\u6a21\u5f0f - \u5b9a\u65f6' + this.curModeMsg.modeNum
				}
				return '\u4e13\u4e1a\u6a21\u5f0f'
			},
		...
		methods: {
			...
			getTimeData(){
				...
				let yearHex = +yearStr.toString(16)//骞?6杩涘埗
				if(yearHex.length<2){
					yearHex = '0'+yearHex
				}
