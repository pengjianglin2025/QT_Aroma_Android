		...
		<view class="timg">
			...
			<view style="position: relative;">
				...
				<view class="tcon" style="font-size: 42rpx;" v-if="curModeMsg.btime&&curModeMsg.etime">
					...
					<!-- <text v-if="is24hour">{{curModeMsg.btime}} - {{curModeMsg.etime}}</text>
					<text v-else>
						<template v-if="curModeMsg.btime&&curModeMsg.etime">
							{{change12H(curModeMsg.btime)}}{{Number(curModeMsg.btime.split(':')[0]) <12? 'AM':'PM'}} - {{change12H(curModeMsg.etime)}}{{Number(curModeMsg.etime.split(':')[0]) <12? 'AM':'PM'}}
						</template>
					</text> -->
				</view>
			</view>
			<!-- <view class="tcon">
				<text style="font-size: 40px;">{{oilEntity.remain}}</text>
				<text style="padding-top: 30rpx;">ML</text>
			</view> -->
			
			<view class="flex justify-center" style="font-size: 28rpx;margin-top: 48rpx;">
				<div class="modetimes" style="border-radius: 30rpx 0 0 30rpx;color: #fff;">{{$t('com.work')}} {{curModeMsg.wtimes||0}}s</div> 
				<div class="modetimes" style="background: #F1F1F1;border-radius:0 30rpx 30rpx 0;">{{$t('com.pause')}} {{curModeMsg.ptimes||0}}s</div>
			</view>
		</view>
		<!-- <view class="timg" v-if="Object.keys(curModeMsg).length != 0">
			<div style="font-size: 50rpx;font-weight: bold;">{{curModeMsg.btime}} - {{curModeMsg.etime}}</div>
			<div style="font-size: 50rpx;font-weight: bold;">{{$t('com.work')}}:{{curModeMsg.wtimes}} {{$t('com.pause')}}:{{curModeMsg.ptimes}}</div>
		</view> -->
		<view class="oilV">
			<view>
				<uni-row>
					...
					<uni-col :span="6">
						<view class="oilItem">
							...
							<view>
								<!-- <image v-if="rssiNum>-60" src="../../../static/signa3.png" mode="aspectFit" style="width: 40rpx;height:40rpx;"></image>
					...
					<view class="funList">
						<view :style="selEntity.tvSelected == true?'background-color:#0BC173':''" @click="handleSelectedIndex(4)">
							<view>
								<view>
									<image
										:src="selEntity.tvSelected == true?'../../../static/television1.png':'../../../static/television.png'"
										mode="heightFix" style="height:60rpx;"></image>
								</view>
								<view class="descV">
									<text
										:style="selEntity.tvSelected == true?'color:#ffffff':''">{{$t('com.bsg')}}</text>
								</view>
							</view>
						</view>
					</view>
...
<script>
	...
	export default {
		...
		computed:{
			...
			...mapGetters(['isOffline','pauseTimeMin','pauseTimeMax','workTimeMin','workTimeMax','workStep','workModelHexStr'])
		},
		onLoad(e) {
			// this.devicemsg = e;
			...
			toWorkModel() {
				...
				if(this.switchSelected){
					uni.navigateTo({
						...
						// url: `/pages/work/pattern/index?deviceKey=${deviceKey}&productKey=${productKey}&deviceName=${deviceName}`
					})
				}else{
					uni.showToast({
						icon:'none',
